...
#define MAX_BTREE_PAGE_KEYS    509
...
- (void) _loadEntries {
    if (self.type == GTWAOFBTreeLeafNodeType) {
        ...
        for (i = 0; i < count; i++) {
            NSData* key = [data subdataWithRange:NSMakeRange(offset, 8)];
            NSData* val = [data subdataWithRange:NSMakeRange(offset+8, 8)];
            [keys addObject:key];
            [vals addObject:val];
            offset      += 16;
        }
...
- (GTWAOFBTreeNode*) childForKey:(NSData*)key {
    ...
    for (i = 0; i < count; i++) {
        NSData* key = _keys[i];
        NSComparisonResult r    = [b64 compare:[key base64EncodedStringWithOptions:0]];
...
NSData* newLeafNodeData( NSUInteger pageSize, NSArray* keys, NSArray* objects, BOOL verbose ) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (klen != vlen) {
            NSLog(@"Key and value are different sizes (%llu <=> %llu)", (unsigned long long)klen, (unsigned long long)vlen);
            return nil;
        }
        
        [data replaceBytesInRange:NSMakeRange(offset, 8) withBytes:[k bytes]];
        offset  += 8;
        
        [data replaceBytesInRange:NSMakeRange(offset, 8) withBytes:[v bytes]];
        offset  += 8;
    }
